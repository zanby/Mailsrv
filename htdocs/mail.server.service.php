...
require_once ENGINE_DIR . '/Server/Service/Bootstrap.php';
require_once ENGINE_DIR . '/Server/WSSecurity.php';
require_once ENGINE_DIR . '/Server/WSSecurity/Bootstrap/UsernameToken.php';

$soap = new DOMDocument();
$soap->load('php://input');

$wss = new Server_WSSecurity( $soap );
$wss->setUsernameToken( new Server_WSSecurity_Bootstrap_UsernameToken() );
$soapServer = new SoapServer( SERVER_URL."/wsdl.php?t=bootstrap", array() );

try { $response = $wss->process(); }
catch ( Exception $e ) { $wss->failed($e->getMessage()); }
if ( !$response ) { $wss->failed('Cannot process soap request'); }

$soapServer->setObject(new Server_Service_Bootstrap());
