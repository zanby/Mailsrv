...
class Dklab_SoapClient_Curl
{
    ...
    public function addRequest($curlOptions)
    {
        ...
        if ( isset($curlOptions['callArgs']) ) {
            $callArgs = $curlOptions['callArgs'];
            unset($curlOptions['callArgs']);
        }        
    ...
    private function _storeResponses(&$nRunning = null)
    {
        while ($done = curl_multi_info_read($this->_handler)) {
            ...
            if (
                ...
                && $request->validator 
                && !call_user_func($request->validator, $response, $request->tries, $request->callArgs)
            ) {
                // Initiate the retry.
                $request->tries++;
