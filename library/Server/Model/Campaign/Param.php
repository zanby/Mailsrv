...
*/
class Server_Model_Campaign_Param
{
    protected $id;
    ...
    {
        if ( is_object($value) ) throw new Exception("Value of param can not be an object");
        if ( is_array($value) ) throw new Exception("Value of param can not be an array");
    ...
    static public function loadByCampaign( $campaignId )
    {
        ...
        $select_sql = "SELECT * FROM mail_campaignes__params WHERE campaign_id = :campaign_id";
        ...
        if ( $results && sizeof($results) ) {
            foreach ( $results as $result ) {
                ...
                $objParam->setId($result['id']);
                $objParam->setCampaignId($result['campaign_id']);            
                $objParam->setName($result['name']);
                $objParam->setValue($result['value']);
                $return[] = $objParam;
            }
            return $return;
        }        
